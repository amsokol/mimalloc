...
```toml
[dependencies]
mimalloc = { git = "https://github.com/amsokol/mimalloc", tag = "v2.3.1" }
```
...
| `override` | Overrides the standard C `malloc`/`free` interface so all C library allocations also use mimalloc |

Enable features in your `Cargo.toml`:

```toml
[dependencies]
mimalloc = { git = "https://github.com/amsokol/mimalloc", tag = "v2.3.1", features = ["secure"] }
```
